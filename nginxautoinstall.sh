...
if [ $TAGINSTALL == 1 ]
then
	displayandexec "Init the default configuration file for NGinx" "$WGET https://raw.githubusercontent.com/92bondstreet/genesis-nginx/master/nginx.conf ; $WGET https://raw.githubusercontent.com/92bondstreet/genesis-nginx/master/default-site ; mv nginx.conf /etc/nginx/ ; mv default-site /etc/nginx/sites-enabled/"
  displayandexec "Init a default file for a succees NGinx installation" "$WGET https://raw.githubusercontent.com/92bondstreet/genesis-nginx/master/genesis-nginx.php ;  mv genesis-nginx.php /var/www/"
fi

# Download the init script
displayandexec "Install the NGinx init script" "$WGET https://raw.githubusercontent.com/92bondstreet/genesis-nginx/master/nginx ; mv nginx /etc/init.d/ ; chmod 755 /etc/init.d/nginx ; /usr/sbin/update-rc.d -f nginx defaults"
...
# update PHP-FPM5 to avoid 502 bad gateway
displayandexec "update PHP-FPM5 to avoid 502 bad gateway" "echo \"listen = 127.0.0.1:9000\" >> /etc/php5/fpm/php-fpm.conf"
